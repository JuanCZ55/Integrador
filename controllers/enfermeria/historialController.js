...
async function postHistoria(req, res) {
  try {
    ...
    res.redirect(
      `/enfermeria/historial?id_admision=${id_admision}&mensajeAlert=Historial actualizado&alertClass=alert-success`
    );
  } catch (error) {
    console.error(error);
    res.redirect(
      `/enfermeria/pacientes?mensajeAlert=Error al guardar el historial&alertClass=alert-danger`
    );
  }
}
